...
est_vocal = zeros(len_x, 1);
for i=1:num_blocks
    temp_sig = ifft([rec_vocal_spec(:,i); conj(flipud(rec_vocal_spec(2:end-1,i)))]);
    temp_sig = real(temp_sig);
    est_vocal((i-1)*hop_size+1:(i-1)*hop_size+fft_size) = est_vocal((i-1)*hop_size+1:(i-1)*hop_size+fft_size) + hann(fft_size).*temp_sig;
end

est_inst = zeros(len_x, 1);
for i=1:num_blocks
    temp_sig = ifft([rec_inst_spec(:,i); conj(flipud(rec_inst_spec(2:end-1,i)))]);
    temp_sig = real(temp_sig);
